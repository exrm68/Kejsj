...
const AdminPanel: React.FC<AdminPanelProps> = ({ onClose }) => {
  ...
  const [channelLink, setChannelLink] = useState('');              // Notice bar link
  const [headerTelegramLink, setHeaderTelegramLink] = useState(''); // Header link
  const [sidebarTelegramLink, setSidebarTelegramLink] = useState(''); // Sidebar link
  const [bannerTelegramLink, setBannerTelegramLink] = useState(''); // Banner link
  ...
  const fetchSettings = async () => {
    try {
      ...
      if (docSnap.exists()) {
        ...
        setHeaderTelegramLink(data.headerTelegramLink || '');
        setSidebarTelegramLink(data.sidebarTelegramLink || '');
        setBannerTelegramLink(data.bannerTelegramLink || '');
                ...
                <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      />
                      <p className="text-xs text-gray-500 mt-1">Video telegram bot এর username</p>
                    </div>

                    {/* Notice Bar Telegram Link */}
                    <div>
                      <label className="block text-sm text-gray-300 mb-2">
                        <Bell size={16} className="inline mr-1 text-yellow-400" />
                        Notice Bar Telegram Link
                      </label>
                      <input
                        type="text"
                      ...
                      />
                      <p className="text-xs text-gray-500 mt-1">Notice bar এ যে Telegram link দেখাবে</p>
                    </div>

                    {/* Header Telegram Link */}
                    <div>
                      <label className="block text-sm text-gray-300 mb-2">
                        <Send size={16} className="inline mr-1 text-blue-400" />
                        Header Telegram Link (Right Side)
                      </label>
                      <input
                        type="text"
                        value={headerTelegramLink}
                        onChange={(e) => setHeaderTelegramLink(e.target.value)}
                        className="w-full px-4 py-2.5 bg-gray-900 border border-gray-700 rounded-lg text-white"
                        placeholder="https://t.me/yourchannel"
                      />
                      <p className="text-xs text-gray-500 mt-1">Header এর ডান পাশে Telegram icon এ যে link থাকবে</p>
                    </div>

                    {/* Sidebar Telegram Link */}
                    <div>
                      <label className="block text-sm text-gray-300 mb-2">
                        <Link size={16} className="inline mr-1 text-green-400" />
                        Sidebar Telegram Link
                      </label>
                      <input
                        type="text"
                        value={sidebarTelegramLink}
                        onChange={(e) => setSidebarTelegramLink(e.target.value)}
                        className="w-full px-4 py-2.5 bg-gray-900 border border-gray-700 rounded-lg text-white"
                        placeholder="https://t.me/yourchannel"
                      />
                      <p className="text-xs text-gray-500 mt-1">Sidebar menu তে Telegram link</p>
                    </div>

                    {/* Banner Telegram Link */}
                    <div>
                      <label className="block text-sm text-gray-300 mb-2">
                        <Zap size={16} className="inline mr-1 text-purple-400" />
                        Banner Telegram Link (Optional)
                      </label>
                      <input
                        type="text"
                        value={bannerTelegramLink}
                        onChange={(e) => setBannerTelegramLink(e.target.value)}
                        className="w-full px-4 py-2.5 bg-gray-900 border border-gray-700 rounded-lg text-white"
                        placeholder="https://t.me/yourchannel"
                      />
                      <p className="text-xs text-gray-500 mt-1">Banner এ যদি Telegram link দিতে চান (optional)</p>
                    </div>
                  </div>
                </div>
